const closingJournalRoute = require('express').Router()
...
// route halaman Akun
closingJournalRoute.get('/', async (req, res) => {
    const journals = await Journal.find()
    const childAccs = await ChildAcc.find()

    res.render('closing-journal', {
...
closingJournalRoute.get('/add', async (req, res) => {
    const journals = await Journal.find()
    ...
    res.render('closing-journal/add', {
        title: 'Transaksi Jurnal Penutup',
        layout: 'layouts/main-layout',
        journals,
        parentAccs,
        childAccs,
...
closingJournalRoute.post('/', [
    ...
    async (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            let data = { date, description, ref: 'cj', transNumber, journalTrans: [] }
            ...
            await Journal.insertMany(data, (error, result) => {
                ...
                res.redirect(`/closing-journal/r/${transNumber}`)
            })
...
// prosses delete data
closingJournalRoute.delete('/', (req, res) => {
    Journal.deleteOne({ _id: req.body._id }).then((result) => {
        req.flash('msg', 'Transaksi berhasil dihapus!')
        res.redirect('/closing-journal')
    })
})

// // halaman edit data
// closingJournalRoute.get('/edit/:accChildName', async (req, res) => {
...
closingJournalRoute.get('/:_id', async (req, res) => {
    const journals = await Journal.findOne({ _id: req.params._id })
    const childAccs = await ChildAcc.find()
    res.render('closing-journal/detail', {
        layout: 'layouts/main-layout',
        title: 'Detail Transaksi Jurnal Penutup',
        journals,
        childAccs
    })
})

closingJournalRoute.get('/r/:transNumber', async (req, res) => {
    const journal = await Journal.findOne({ transNumber: req.params.transNumber })
    const id = journal._id
    res.redirect(`/closing-journal/${id}`)
})
...
module.exports = closingJournalRoute